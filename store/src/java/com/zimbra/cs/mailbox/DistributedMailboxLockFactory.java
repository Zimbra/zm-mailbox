...
public class DistributedMailboxLockFactory implements MailboxLockFactory {
    ...
    @Override
    public MailboxLock readLock(MailboxLockContext lockContext) {
        return new DistributedMailboxLock(localLock, redisLock.readLock(lockContext), false, lockContext);
    }

    @Override
    public MailboxLock writeLock(MailboxLockContext lockContext) {
        return new DistributedMailboxLock(localLock, redisLock.writeLock(lockContext), true, lockContext);
    }
    ...
    @Override
    @Deprecated
    public MailboxLock lock(MailboxLockContext lockContext, final boolean write) {
        if (write || this.mailbox.requiresWriteLock()) {
            return writeLock(lockContext);
        }
        return readLock(lockContext);
    }
    ...
    public class LocalMailboxLock implements MailboxLock {
        ...
        protected final MailboxLockContext lockContext;

        private LocalMailboxLock(final ReentrantReadWriteLock localLock, final boolean write, final MailboxLockContext lockContext) {
    ...
    public class DistributedMailboxLock extends LocalMailboxLock {
        ...
        public DistributedMailboxLock(ReentrantReadWriteLock localLock, RedisLock zRedisLock, boolean write, MailboxLockContext lockContext) {
            super(localLock, write, lockContext);
            this.zRedisLock = zRedisLock;
            this.write = write;
        }

        @Override
        public synchronized void lock() throws ServiceException {
            if (getHoldCount() == 0) {
                ...
                if (Strings.isNullOrEmpty(lastWriter)) {
                    if (ZimbraLog.cache.isDebugEnabled()) {
                        ZimbraLog.cache.debug("unable to determine last mailbox worker to acquire write lock for %s, flushing local caches", mailbox.getAccount().getName());
                    }
                    mailbox.clearStaleCaches(lockContext);
                } else if (!lastWriter.equals(lockNode)) {
                    if (write) {
                        ZimbraLog.cache.info("last write lock for %s was acquired by different mailbox worker (%s), flushing local caches prior to write lock (caller=%s)",
                                mailbox.getAccount().getName(), lastWriter, lockContext.getCaller());
                        mailbox.clearStaleCaches(lockContext);
                    } else if (!LC.only_flush_cache_on_first_read_after_foreign_write.booleanValue() || response.isFirstReadSinceLastWrite()) {
                        ZimbraLog.cache.info("last write lock for %s was acquired by different mailbox worker (%s), flushing local caches prior to read lock (caller=%s)",
                                mailbox.getAccount().getName(), lastWriter, lockContext.getCaller());
                        mailbox.clearStaleCaches(lockContext);
                    } else {
                        if (ZimbraLog.cache.isDebugEnabled()) {
        ...
        protected boolean releaseReadLocksBeforeWriteLock() {
            ...
            if (releaseRedisReadLock) {
                ...
                redisLock.readLock(lockContext).unlock();
            }
                ...
                try {
                    ...
                    if (reinstateRedisReadLock) {
                        try {
                            ...
                            redisLock.readLock(lockContext).lock(true);
