...
public class Auth extends AdminDocumentHandler {
	...
	public Element handle(Element request, Map<String, Object> context) throws ServiceException {
		...
		if (name == null && acctEl == null) {
			...
			// make sure that the authenticated account is active and has not been
			// deleted/disabled since the last request
			...
			/*
			 * only one of <name>...</name> or <account
			 * by="name|id|foreignPrincipal">...</account> can/must be specified
			 */
			if (name != null && acctEl != null)
			...
			try {
				...
				if (by == AccountBy.name && value.indexOf("@") == -1) {
					// first try to get by adminName, which resolves the account under
					// cn=admins,cn=zimbra
				...
				// Only perform 2fa authentication if account has server
				// Zmprov account will not have any server
				if (acct != null && acct.getServer() != null) {
					...
					if (password != null || recoveryCode != null || twoFactorAuthWithToken) {
						...
						if (usingTwoFactorAuth && twoFactorCode == null && (password != null || recoveryCode != null)) {
							...
							if (supportsAppSpecificPaswords && password != null) {
								...
								AppSpecificPasswords appPasswords = TwoFactorAuth.getFactory()
										.getAppSpecificPasswords(acct, acctValuePassedIn);
							...
							if (usingTwoFactorAuth) {
								...
								if (password == null) {
									try {
										...
										Account twoFactorTokenAcct = AuthProvider.validateAuthToken(prov,
												twoFactorToken, false, Usage.TWO_FACTOR_AUTH);
										boolean verifyAccount = authTokenEl
												.getAttributeBool(AccountConstants.A_VERIFY_ACCOUNT, false);
										if (verifyAccount
												&& !twoFactorTokenAcct.getId().equalsIgnoreCase(acct.getId())) {
					...
					if (registerTrustedDevice && (trustedToken == null || trustedToken.isExpired())) {
						...
						Map<String, Object> attrs = getTrustedDeviceAttrs(zsc,
								newDeviceId == null ? deviceId : newDeviceId);
						if (trustedDeviceManager != null) {
				...
				ZimbraLog.security.warn(ZimbraLog
						.encodeAttrs(new String[] { "cmd", "AdminAuth", "account", value, "error", se.getMessage() }));
				throw se;
			}
		}
		if (at != null) {
	...
	private void verifyTrustedDevice(Account account, TrustedDeviceToken td, Map<String, Object> attrs)
	...
	private Element needTwoFactorAuth(Account account, TwoFactorAuth auth, ZimbraSoapContext zsc, TokenType tokenType)
			throws ServiceException {
		/*
		 * two cases here: 1) the user needs to provide a two-factor code. in this case,
		 * the server returns a two-factor auth token in the response header that the
		 * client must send back, along with the code, in order to finish the
		 * authentication process. 2) the user needs to set up two-factor auth. this can
		 * happen if it's required for the account but the user hasn't received a secret
		 * yet.
		 */
		if (!auth.twoFactorAuthEnabled()) {
			...
			response.addAttribute(AccountConstants.E_LIFETIME, twoFactorToken.getExpires() - System.currentTimeMillis(),
					Element.Disposition.CONTENT);
			twoFactorToken.encodeAuthResp(response, false);
			response.addUniqueElement(AccountConstants.E_TRUSTED_DEVICES_ENABLED)
	...
	private Element doResponse(Element request, AuthToken at, ZimbraSoapContext zsc, Map<String, Object> context,
			Account acct, boolean csrfSupport) throws ServiceException {
		Element response = zsc.createElement(AdminConstants.AUTH_RESPONSE);
		at.encodeAuthResp(response, true);

		/*
		 * bug 67078 also return auth token cookie in http header
		 */
		...
		response.addAttribute(AdminConstants.E_LIFETIME, at.getExpires() - System.currentTimeMillis(),
				Element.Disposition.CONTENT);
