...
import com.zimbra.cs.service.sso.SsoTwoFactorGate;
...
public class PreAuthServlet extends ZimbraServlet {
    ...
    public static final String PARAM_TFA = SsoTwoFactorGate.PARAM_TFA;
    public static final String PARAM_TFA_EMAIL = SsoTwoFactorGate.PARAM_TFA_EMAIL;
    // Set when 2FA is required but has never been set up: tells the client to open enrolment.
    public static final String PARAM_TFA_ENROLL = SsoTwoFactorGate.PARAM_TFA_ENROLL;
        ...
        try {
            ...
            if (rawAuthToken != null) {
                ...
                if (admin || !needReferral(acct, referMode, isRedirect)) {
                    ...
                    if (!admin && SsoTwoFactorGate.gate(req, resp, acct, "PreAuth")) {
                        return;
                    }
