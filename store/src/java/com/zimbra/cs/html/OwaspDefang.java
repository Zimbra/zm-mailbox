...
public class OwaspDefang extends AbstractDefang {
    ...
    private String runSanitizer(String html, boolean neuterImages) {
        ...
        try {
            sanitizedHtml = future.get(10, TimeUnit.SECONDS);
        } catch (InterruptedException | ExecutionException | TimeoutException e) {
            ZimbraLog.soap.error("Exception during HTML sanitization", e);
            return null;
        }
        executor.shutdown();
...
class OwaspHtmlSanitizer implements Callable<String> {
    ...
    private static final CssSchema ADDITIONAL_CSS = CssSchema
        .withProperties(ImmutableSet.of("float", "display"));
        ...
        public String apply(String elementName, String attributeName, String value) {
            if (sameHostFormPostCheck == true && reqVirtualHost != null) {
                try {
                    ...
                    ZimbraLog.soap
                        .error("Failure while trying to block mailicious code. Check for URL "
    ...
    private static PolicyFactory POLICY_DEFINITION;
    ...
    public String sanitize() {
        ...
        org.owasp.html.HtmlSanitizer.sanitize(html, policy);
