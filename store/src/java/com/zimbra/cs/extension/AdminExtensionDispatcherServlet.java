...
/**
 * Registers {@link ExtensionHttpHandler} for admin extensions and dispatches HTTP requests to them.
 * This servlet accepts requests sent to /service/admin/extension/&lt;ext name>/&lt;handler path> URI,
 * where &lt;ext name>/&lt;handler path> is used to locate the handler whose {@link ExtensionHttpHandler#getPath() }
 * has the same path.
 *
 */
public class AdminExtensionDispatcherServlet extends ZimbraServlet {
    private static final long serialVersionUID = 5008451018337533570L;

    private static Map sHandlers = Collections.synchronizedMap(new HashMap());
    public static final String EXTENSION_PATH = "/service/admin/extension";
    ...
    public void service(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        try {
            ...
            ZimbraLog.extensions.warn("unable to find handler for extension: %s", e.getMessage());
    ...
    private ExtensionHttpHandler getHandler(HttpServletRequest req) throws ServiceException {
        ...
        ZimbraLog.extensions.debug("getting handler registered at %s", extPath);
