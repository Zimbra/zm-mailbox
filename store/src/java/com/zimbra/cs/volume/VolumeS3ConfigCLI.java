...
public final class VolumeS3ConfigCLI extends SoapCLI {
    ...
    private static final String AWS_S3 = "AWS_S3";
    private static final String CEPH_S3 = "CEPH_S3";
    private static final String STORAGE_GRID_S3 = "CEPH_S3";
    // variables
    ...
    /**
     * This method prints option
     *
     * @param optStr, leftPad
     */
    ...
    private void validateAddCommand() throws ParseException {
        validateStoreProvider();
        if (storeProvider.equalsIgnoreCase(AWS_S3)) {
            validateS3Params();
        } else if (storeProvider.equalsIgnoreCase(CEPH_S3)) {
            validateCephParams();
        } else if (storeProvider.equalsIgnoreCase(STORAGE_GRID_S3)) {
            validateStorageGridParams();
        } else {
            throw new ParseException("Incorrect Store Provider, available Store Provider are 1." + AWS_S3  + " 2." + CEPH_S3 + " 3." + STORAGE_GRID_S3);
        }
    }
    ...
    private void addS3BucketConfig() throws IOException, ServiceException, ParseException {
        ...
        String bid = "";
        for (Attr item: resp_CreateS3BucketConfigRequest().getAttrs()) {
        ...
        if (StringUtil.isNullOrEmpty(bid) || !isS3BucketConfigExists(bid)) {
            ...
            System.out.println("Success: s3BucketConfig successfully created");
        }
    }
    ...
    private void delS3BucketConfig() throws IOException, ServiceException, ParseException {
        ...
        if (isS3BucketConfigExists(bucketId)) {
            ...
            System.out.println("Success: s3BucketConfig successfully deleted");
        }
    }
    ...
    private void lstS3BucketConfigs() throws IOException, ServiceException {
        for (GlobalS3BucketConfiguration item : resp_GetS3BucketConfigRequest().getGlobalS3BucketConfigurations()) {
    ...
    private GetS3BucketConfigResponse resp_GetS3BucketConfigRequest() throws IOException, ServiceException {
        ...
        auth(auth);
        return JaxbUtil.elementToJaxb(getTransport().invokeWithoutSession(JaxbUtil.jaxbToElement(s3BucketConfigRequest)));
    }
