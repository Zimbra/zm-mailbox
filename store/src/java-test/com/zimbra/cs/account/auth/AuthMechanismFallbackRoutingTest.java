...
public class AuthMechanismFallbackRoutingTest {
    ...
    public void setUp() throws Exception {
        ...
        registerMockIdpRopcHandler();
    }

    /**
     * Registers a mock "idp-ropc" ZimbraCustomAuth handler that only supports the
     * EAS sub-protocol over zsync, mimicking IRopcCustomAuth.supportsContext() without
     * pulling in the zm-mfa-ext module as a test dependency.
     * ZimbraCustomAuth.register() is a no-op (with a warning log) if the name is already
    ...
    private void registerMockIdpRopcHandler() {
        ZimbraCustomAuth.register(IDP_ROPC_HANDLER_NAME, new ZimbraCustomAuth() {
            ...
            @Override
            public boolean supportsContext(Map<String, Object> context) {
                if (context == null) {
                    return false;
                }
                Object protocol = context.get(AuthContext.AC_PROTOCOL);
                Object subProtocol = context.get(AuthContext.AC_SUB_PROTOCOL);
                return AuthContext.Protocol.zsync.equals(protocol)
                        && AuthContext.SubProtocol.eas.equals(subProtocol);
            }
        });
    }
