...
public class MapUtil {
    ...
    /**
     * Converts a Guava {@code Multimap} to a {@code LoadingCache} that maps a key to a
     * {@code List} of values.
     */
    public static <K, V> LoadingCache<K, List<V>> multimapToMapOfLists(Multimap<K, V> multimap) {
        LoadingCache<K, List<V>> map = newValueListMap();
        if (multimap != null) {
            for (Map.Entry<K, V> entry : multimap.entries()) {
                try {
                    map.get(entry.getKey()).add(entry.getValue());
                } catch (ExecutionException e) {
                    ZimbraLog.cache.warn("Unable to load value for %s: %s", entry.getKey(),
                        e.getMessage());
                }
            }
        }
