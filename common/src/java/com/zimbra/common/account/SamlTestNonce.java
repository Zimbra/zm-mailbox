...
public final class SamlTestNonce {
    ...
    /**
     * Returns {@code true} if the candidate nonce matches the stored nonce value but has expired
     * (i.e. the 30-minute window elapsed). Used to detect abandoned/late test completions so we
     * can record a "nonce expired" result rather than silently treating it as a normal login.
     *
     * @param candidateNonce nonce extracted from RelayState
     * @param storedValue    raw {@code zimbraSamlTestNonce} value from LDAP
     * @param nowMillis      current time in epoch millis
     * @return {@code true} if the nonce matches but is past its {@code notAfter} deadline
     */
    public static boolean isMatchedButExpired(String candidateNonce, String storedValue, long nowMillis) {
        if (StringUtil.isNullOrEmpty(candidateNonce)) {
            return false;
        }
        final Stored stored = parseStoredValue(storedValue);
        ...
        if (nowMillis < stored.getNotAfterMillis()) {
            return false; // not expired — isValid() would handle this case
        }
        // Nonce value must match (constant-time comparison)
        return MessageDigest.isEqual(
                candidateNonce.getBytes(StandardCharsets.UTF_8),
                stored.getNonce().getBytes(StandardCharsets.UTF_8));
    }
