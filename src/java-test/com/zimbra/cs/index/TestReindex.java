 ...
 * test various re-indexing scenarios (w/o SOAP)
 * This tests use local (default) indexing queue and EmbeddedSolrServer
 */
public class TestReindex {
    ...
    public void testReIndexAllMessages() throws Exception {
        ...
        IndexingQueueAdapter queueAdapter = Zimbra.getAppContext().getBean(IndexingQueueAdapter.class);
        assertEquals("at this point total count should be 4", 4, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 0", 0, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ...
        assertEquals("at this point should have succeeded re-indexing 4 items", 4, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ids = TestUtil.search(mbox, "from:greg", MailItem.Type.MESSAGE);
    ...
    public void testReIndexContacts() throws Exception {
        ...
        IndexingQueueAdapter queueAdapter = Zimbra.getAppContext().getBean(IndexingQueueAdapter.class);
        assertEquals("at this point total count should be 1", 1, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 0", 0, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ...
        assertEquals("at this point total count should still be 1", 1, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 1", 1, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        
        ids = TestUtil.search(mbox, "from:greg", MailItem.Type.MESSAGE);
        assertEquals("Should not be able to find messages after re-indexing Contacts only", 0, ids.size());
        
        Assert.assertTrue("should be able to find the contact after re-indexing Contacts only", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("Test <test1@zimbra.com>"), new InternetAddress("Test <test2@zimbra.com>"))));
    }
    ...
    public void testReIndexMessages() throws Exception {
        ...
        Assert.assertFalse("should not be able to find contacts at this point", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("Test <test1@zimbra.com>"), new InternetAddress("Test <test2@zimbra.com>"))));
        ...
        //check results
        assertEquals("at this point total count should be  4", 4, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 4", 4, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ids = TestUtil.search(mbox, "from:greg", MailItem.Type.MESSAGE);
        assertEquals("should find 4 messages after re-indexing Messages only", 4, ids.size());
        Assert.assertFalse("should not be able to find contacts after re-indexing Messages only", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("Test <test1@zimbra.com>"), new InternetAddress("Test <test2@zimbra.com>"))));
    }
    ...
    public void testReIndexAppointments() throws Exception {
        ...
        createCalendarItem("Shall we dance?", "We need a witness to our lives. There's a billion people on the planet...", "Hollywood");
        ...
        Assert.assertFalse("should not be able to find contacts after re-indexing appointments only", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("Test <test1@zimbra.com>"), new InternetAddress("Test <test2@zimbra.com>"))));
        ids = TestUtil.search(mbox, "dance", MailItem.Type.APPOINTMENT);
    ...
    public void testReIndexAll() throws Exception {
        ...
        IndexingQueueAdapter queueAdapter = Zimbra.getAppContext().getBean(IndexingQueueAdapter.class);
        assertEquals("at this point total count should be 6", 6, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 0", 0, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ...
        //check search results
        assertEquals("at this point total count should be 6", 6, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 6", 6, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ids = TestUtil.search(mbox, "from:greg", MailItem.Type.MESSAGE);
        assertEquals("should find messages after re-indexing everything", 4, ids.size());
        Assert.assertTrue("should find contacts after re-indexing everything", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("Test <test1@zimbra.com>"), new InternetAddress("Test <test2@zimbra.com>"))));
        ids = TestUtil.search(mbox, "dance", MailItem.Type.APPOINTMENT);
    ...
    public void testReIndexAllIn3Batches() throws Exception {
        ...
        //now add some contacts
        mbox.createContact(null, new ParsedContact(Collections.singletonMap(
                ContactConstants.A_email, "david.gilmour@zimbra.com")), Mailbox.ID_FOLDER_CONTACTS, null);
        mbox.createContact(null, new ParsedContact(Collections.singletonMap(
                ContactConstants.A_email, "waters@zimbra.com")), Mailbox.ID_FOLDER_CONTACTS, null);
        ...
        Assert.assertEquals("should find 4 messsages", 4, ids.size());
        Assert.assertTrue("should find the new contact", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("David <david.gilmour@zimbra.com>"), new InternetAddress("Roger <waters@zimbra.com>"))));
        ...
        IndexingQueueAdapter queueAdapter = Zimbra.getAppContext().getBean(IndexingQueueAdapter.class);
        assertEquals("at this point total count should be 7", 7, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 0", 0, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ...
        //check search results
        assertEquals("at this point total count should be 7", 7, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 7", 7, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ids = TestUtil.search(mbox, "from:greg", MailItem.Type.MESSAGE);
        assertEquals("should find messages after re-indexing everything", 4, ids.size());
        Assert.assertTrue("should find the new contact", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("David <david.gilmour@zimbra.com>"), new InternetAddress("Roger <waters@zimbra.com>"))));
    ...
    public void testInvalidBatchSize() throws Exception {
        ...
        Assert.assertFalse("should not find contacts when re-index batch is 0", mbox.index.existsInContacts(ImmutableList.of(
                new InternetAddress("Test <test1@zimbra.com>"), new InternetAddress("Test <test2@zimbra.com>"))));
        ids = TestUtil.search(mbox, "dance", MailItem.Type.APPOINTMENT);
    ...
    public void testReIndexByID() throws Exception {
        ...
        IndexingQueueAdapter queueAdapter = Zimbra.getAppContext().getBean(IndexingQueueAdapter.class);
        assertEquals("at this point 'total' count should be 2", 2, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 0", 0, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ...
        assertEquals("at this point 'total' count should be 2", 2, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("at this point 'succeeded' count should be 2", 2, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ids = TestUtil.search(mbox, "from:greg", MailItem.Type.MESSAGE);
    ...
    public void testReindexStatus() throws Exception {
        ...
        IndexingQueueAdapter queueAdapter = Zimbra.getAppContext().getBean(IndexingQueueAdapter.class);
        assertEquals("'total' queue should have 4 items before re-index starts", 4, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("'succeeded' count should have 0 items before re-index starts", 0, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        ...
        assertFalse("ReIndexStatus.cancel should be false",status.isCancelled());
        ...
        assertEquals("'succeeded' count should be 4 after re-index is finished", 4, queueAdapter.getSucceededMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
        assertEquals("'total' count should be 4 after re-index is finished", 4, queueAdapter.getTotalMailboxTaskCount(MockProvisioning.DEFAULT_ACCOUNT_ID));
    ...
    public void cancelReIndex() throws Exception {
        ...
        mbox.index.cancelReIndex();
        status = mbox.index.getReIndexStatus();
        assertNotNull("ReIndexStatus should not be null", status);
        assertEquals("ReIndexStatus should contain -1 total items", -1,status.getTotal());
        assertEquals("ReIndexStatus should contain 0 processed items", 0,status.getSucceeded());
        assertTrue("ReIndexStatus.cancel should be true",status.isCancelled());
        ...
        assertEquals("ReIndexStatus should contain -1 total items", -1, status.getTotal());
        assertEquals("ReIndexStatus should contain 0 processed items", 0, status.getSucceeded());
        assertTrue("ReIndexStatus.cancel should be false",status.isCancelled());
    ...
    private List<MailItem> create4Messages() throws Exception {
        ...
        mailItems.add(mbox.addMessage(null, new ParsedMessage(
                "From: greg@zimbra.com\r\nTo: test@zimbra.com\r\nSubject: And summer's lease hath all too short a date".getBytes(), false), dopt, null));
