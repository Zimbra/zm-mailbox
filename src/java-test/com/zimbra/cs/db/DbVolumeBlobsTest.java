...
public class DbVolumeBlobsTest {
    ...
    public void writeAllBlobRefs() throws Exception {
        ...
        Assert.assertEquals(msgDigests.size(), blobs.size());
        for (BlobReference blob : blobs) {
            Assert.assertTrue(msgDigests.remove(blob.getDigest()));
        }

        Assert.assertTrue(msgDigests.isEmpty());
    }
