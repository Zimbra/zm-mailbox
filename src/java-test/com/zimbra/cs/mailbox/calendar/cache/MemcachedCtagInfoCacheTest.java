...
/**
 * Unit test for {@link MemcachedCtagInfoCache}.
 */
public final class MemcachedCtagInfoCacheTest extends AbstractCtagInfoCacheTest {
    ...
    protected CtagInfoCache constructCache() throws ServiceException {
        CtagInfoCache cache = new MemcachedCtagInfoCache();
        Zimbra.getAppContext().getAutowireCapableBeanFactory().autowireBean(cache);
        Zimbra.getAppContext().getAutowireCapableBeanFactory().initializeBean(cache, "ctagInfoCache");
        return cache;
    }
