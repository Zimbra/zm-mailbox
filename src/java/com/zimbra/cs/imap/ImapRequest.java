...
class ImapRequest {
    ...
    int readFetch(List parts) throws IOException, ImapException {
        ...
        do {
            ...
            if (!list && item.equals("ALL"))        attributes = ImapHandler.FETCH_ALL;
            ...
            else if (item.equals("RFC822.HEADER"))  parts.add(new PartSpecifier(item, "", "HEADER"));
            else if (item.equals("RFC822")) {
                attributes |= ImapHandler.FETCH_MARK_READ;
                parts.add(new PartSpecifier(item, "", ""));
            } else if (item.equals("RFC822.TEXT")) {
                attributes |= ImapHandler.FETCH_MARK_READ;
                parts.add(new PartSpecifier(item, "", "TEXT"));
                ...
                PartSpecifier pspec = new PartSpecifier("BODY", sectionPart, sectionText, partialStart, partialCount);
