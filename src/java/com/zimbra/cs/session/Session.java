...
public abstract class Session {
    ...
    protected volatile MailboxListenerTransport multiServerMailboxNotifyAdapter;
    ...
    public enum Type {
        ...
        SOAP(1, ProvisioningUtil.getServerAttribute(ZAttrProvisioning.A_zimbraAdminSessionLimit, 5)),
        IMAP(2, Math.max(0, ProvisioningUtil.getServerAttribute(ZAttrProvisioning.A_zimbraImapSessionLimit, 5))),
        ADMIN(3, ProvisioningUtil.getServerAttribute(ZAttrProvisioning.A_zimbraAdminSessionLimit, 5)),
        WIKI(4, 0),
        SYNCLISTENER(5, ProvisioningUtil.getServerAttribute(ZAttrProvisioning.A_zimbraSyncSessionLimit, 5)),
        WAITSET(6, 0)
        ;
    ...
    public Session register() throws ServiceException {
        ...
        if (isMailboxListener()) {
            ...
            if (config != null) {
                try {
                    List<MailboxListenerTransport> list = config.externalMailboxListeners();
                    if (list != null) {
                        for (MailboxListenerTransport manager: list) {
    ...
    public Session unregister() {
        ...
        if (multiServerMailboxNotifyAdapter != null) {
            multiServerMailboxNotifyAdapter.unsubscribe(this);
            multiServerMailboxNotifyAdapter = null;
        }
