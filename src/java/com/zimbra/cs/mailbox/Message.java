        ...
        for (Invite cur : invites) {
            ...
            if (ICalTok.REQUEST.toString().equals(method) ||
                ...
                if (octxt != null) {
                    ...
                    canInvite = accessMgr.canPerform(authAcct, getAccount(), Right.invite, octxt.isUsingAdminPrivileges(), allowInviteIfNoAceDefined);
                } else {
                    senderEmail = pm != null ? pm.getSenderEmail(false) : null;
                    canInvite = accessMgr.canPerform(senderEmail, getAccount(), Right.invite, false, allowInviteIfNoAceDefined);
                }
                if (!canInvite) {
