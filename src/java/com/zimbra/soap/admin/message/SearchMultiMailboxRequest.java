 ...
 * <br />
 * Some search parameters will behave strangely when use across multiple mailboxes.  In particular, Folders and Tags
 * are local to the account making the search (NOT the account being searched in) -- so things like "in:inbox" will
 * not return extra results even if multiple mailboxes are specified.  In general this API is intended to be used for
