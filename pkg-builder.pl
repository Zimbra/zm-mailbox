   ...
   if ( $pkg_info->{file_list} )
   {
      foreach my $expr ( @{ $pkg_info->{file_list} } )
      {
         ...
         foreach my $entry (`find $dir_expr -type f`)
         {
            ...
            push( @cmd, "--pkg-install-list=$entry" );
         }
      }
   }

   push( @cmd, "--pkg-depends-list=$pkg_info->{service_group}" ) if ( $pkg_info->{service_group} );
   push( @cmd, @{ [ map { "--pkg-depends-list=$_"; } @{ $pkg_info->{other_deps} } ] } )                                                             if ( $pkg_info->{other_deps} );
   push( @cmd, @{ [ map { "--pkg-depends-list=$_ (>= $PKG_GRAPH{$_}->{version})"; } @{ $pkg_info->{soft_deps} } ] } )                               if ( $pkg_info->{soft_deps} );
   push( @cmd, @{ [ map { "--pkg-depends-list=$_ (= $PKG_GRAPH{$_}->{_version_ts}-$PKG_GRAPH{$_}->{revision})"; } @{ $pkg_info->{hard_deps} } ] } ) if ( $pkg_info->{hard_deps} );
